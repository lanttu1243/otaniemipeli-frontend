...
export default function AddPlaceToBoard({boardId}: {boardId: number}): JSX.Element {
  const [places, setPlaces] = useState<Places>({ places: [] });
  const [boardPlaces, setBoardPlaces] = useState<BoardPlaces>({ board: { id: boardId, name: '' }, places: [] });
  ...
  useEffect(() => {
    getPlacesNotInBoard(boardId).then(
      (data: {p: Places, bp: BoardPlaces}) => {
        setPlaces(data.p);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-4 border rounded-lg max-w-md mx-auto">
      ...
      <div className="flex gap-2">
        <label className="flex items-center gap-1">
          <input type="checkbox" checked={start} onChange={() => !end ? setStart(!start) : {}} />
          Start
        </label>
        <label className="flex items-center gap-1">
          <input type="checkbox" checked={end}  onChange={() => !start ? setEnd(!end) : {}} />
          End
        </label>
      </div>
